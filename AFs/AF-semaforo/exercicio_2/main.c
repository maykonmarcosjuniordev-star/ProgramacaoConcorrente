...
void ponte_inicializar(int total_veiculos, int veiculos_turno) {
	...
	// Semáforos
	sem_init(&semaforos[0], 0, veiculos_turno);
	sem_init(&semaforos[1], 0, 0);
	

	pthread_mutex_init(&mutex, NULL);
	veiculos_passando = veiculos_turno;
...
void ponte_sair(veiculo_t *v) {
	...
	if (!atravessando) {
		...
		for (int i = 0; i < veiculos_turno; i++) {
			sem_post(&semaforos[v->cabeceira]);
		}
	}
	fflush(stdout);
}
...
void ponte_finalizar() {
	...
	// ToDo: IMPLEMENTAR!
    sem_destroy(&semaforos[0]);
    sem_destroy(&semaforos[1]);
